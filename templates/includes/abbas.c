...
int main(int argc, char **args)
{
    if (argc != 4)
    {
                    printf("\nKullanım Hatası!\nKullanım Şekli:\t uyuyan-berber <Müşteri Sayısı>
                        <Sandalye Sayısı>
                            <Koltuk Sayısı>\n\n");
                                return EXIT_FAILURE;
    }
    ...
    {
        pthread_create(&berber[i], NULL,
                       (void *)ip_berber, (void *)&i);
        sleep(1);
    } /* müşteri iş parçalarının oluşturulması
                                    */
    for (int i = 0; i < musteriSayisi; i++)
    {
        pthread_create(&musteri[i], NULL,
                       (void *)ip_musteri, (void *)&i);
        bekle(); /* müşterileri rastgele aralıklarda
                                    oluşturmak için */
    ...
    for (int i = 0; i < musteriSayisi; i++)
    {
        pthread_join(musteri[i], NULL);
                                    ...
                                    müşterilere hizmet verildi. Berber dükkanı kapandı. Berberler dükkandan
                                    ayrıldı.\n\n"); return EXIT_SUCCESS; } void ip_berber(void* sayi) { int
                                    s=*(int*)sayi + 1; int sonrakiMusteri, musteri_kimligi; printf("[Berber:
                                    %d]\tdükkana geldi.\n", s); while (1) { if (!musteri_kimligi) { printf("[Berber:
                                    %d]\tuyumaya gitti.\n\n", s); } sem_wait(&berberler); /* uyuyan berberlerin
                                    kuyruğuna katıl */ sem_wait(&mutex); /* koltuğa erişimi kilitle */ /* hizmet
                                    edilecek müşterinin bekleyenlerin arasından seçilmesi */
                                    hizmetEdilecekMusteri=(++hizmetEdilecekMusteri) % sandalyeSayisi;
                                    sonrakiMusteri=hizmetEdilecekMusteri; musteri_kimligi=koltuk[sonrakiMusteri];
                                    koltuk[sonrakiMusteri]=pthread_self(); sem_post(&mutex); /* koltuğa erişim kilidini
                                    kaldır */ sem_post(&musteriler); /* seçilen müşteriyle ilgilen */ printf("[Berber:
                                    %d]\t%d. müşterinin saçını kesmeye başladı.\n\n", s, musteri_kimligi);
                                    sleep(KESME_SURESI); printf("[Berber: %d]\t%d. müşterinin saçını kesmeyi
                                    bitirdi.\n\n", s, musteri_kimligi); } } void ip_musteri(void* sayi) { int
                                    s=*(int*)sayi + 1; int oturulanSandalye, berber_kimligi; sem_wait(&mutex); /*
                                    koltuğu korumak için erişimi kilitle */ printf("[Müşteri: %d]\tdükkana geldi.\n",
                                    s); /* bekleme odasında boş sandalye varsa */ if (bosSandalyeSayisi> 0)
                                    {
        bosSandalyeSayisi--;
